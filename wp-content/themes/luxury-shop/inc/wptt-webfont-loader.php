...
if ( ! class_exists( 'Luxury_Shop_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Luxury_Shop_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$luxury_shop_contents = wp_remote_retrieve_body( $response );

			return $luxury_shop_contents;
		}
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $url ) {
					...
					// Get the filename.
					$luxury_shop_filename  = basename( wp_parse_url( $url, PHP_URL_PATH ) );
					$font_path = $folder_path . '/' . $luxury_shop_filename;
...
if ( ! function_exists( 'luxury_shop_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function luxury_shop_wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Luxury_Shop_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'luxury_shop_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function luxury_shop_wptt_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Luxury_Shop_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
